...
namespace GSTAPP.REP
{
    public class SIREP
    {
        ...
        public SalesInvoiceService si = new SalesInvoiceService();
        public ReturnModel<SalesInvoiceViewModel> GetSIPDF(SalesInvoiceViewModel model, string url)
        {
            ...
            var mySI = si.GetSingle(model).datam;
            if (mySI == null)
            {
                return new ReturnModel<SalesInvoiceViewModel>()
                {
            ...
            using (System.IO.FileStream fs = new FileStream(url + "Reports/" + model.InvoiceNo + ".pdf", FileMode.Create))
            {
                ...
                datalist.Add("SI", mySI.InvoiceNo);
                datalist.Add("Invoice Date", mySI.InvoiceDate.Value.ToString("dd/MM/yyyy"));
                datalist.Add("Due Date", mySI.DueDate.Value.ToString("dd/MM/yyyy"));
              
                var tms = new TermsService().GetSingle(new TermsMaster() { Code = mySI.TermsId }).datam;

                Dictionary<string, string> bdatalist = new Dictionary<string, string>();
                bdatalist.Add("Total Amount", mySI.TotalAmount);
                if (mySI.Discount != null && mySI.Discount.Length > 0)
                    bdatalist.Add("Discount", mySI.Discount);
                if (mySI.Message != null && mySI.Message.Length > 0)
                    bdatalist.Add("Message", mySI.Message);


                Image logo = null;
                if (myCom.Image != null && myCom.Image.Length > 1)
                    logo = Image.GetInstance(Convert.FromBase64String(myCom.Image.Split(',')[1]));
                writer.PageEvent = new HeadeFooter(logo, myCom.CompanyName, myCom.Address + " " + myCom.State + " " + myCom.PostalCode + "\n" + myCom.GSTIN, myCus.CompanyName, myCus.Address + " " + myCus.Place + " " + myCus.State + " " + myCus.PostalCode + "\n" + myCus.GSTIN, "Sales Invoice", mySI.ShippingAddress, datalist);     
                ...
                //});
                int ttn = 1;
                mySI.SalesInvoiceItems.ForEach(o =>
                {
                    Paragraph p1 = new Paragraph(new Chunk(mySI.SalesInvoiceItems.IndexOf(o) + 1 + "", normalFont));
                    PdfPCell pcl1 = new PdfPCell();
                    p1.Alignment = Element.ALIGN_LEFT;
                    pcl1.AddElement(p1);
                    pcl1.Border = 0;
                    ...
                    if (ttn == 16)
                    {
                        ...
                        if (mySI.SalesInvoiceItems.IndexOf(o) + 1 != mySI.SalesInvoiceItems.Count)
                        {
                            document.NewPage();
                        }
                    }
